...
const sortByJoinIndex = (players) =>
  Object.values(players).sort((a, b) => a.joinIndex - b.joinIndex);

const BrowserLobby = () => {
  const debug = useContext(DebugContext);

  const [players, setPlayers] = useState(
    debug ? sortByJoinIndex(samplePlayers) : []
  );
  const [sessionName, setSessionName] = useState(debug ? "TEST" : "");
  const [leaderboard, setLeaderboard] = useState(
    debug ? sampleLeaderboard : []
  );
  ...
  const handleStartSelfHostedGame = useCallback(() => {
    socket.emit("start_game_self_hosted");
  });
      ...
      {mute && (
        <MuteScreen>
          <MuteScreenText onClick={() => handleUnmute()}>
            <MuteScreenButton variant={"outline-light"}>
              CLICK TO UNMUTE
            </MuteScreenButton>
          </MuteScreenText>
        </MuteScreen>
            ...
            <Form onSubmit={handleEmail}>
              ...
              >
                ...
                <Form.Control
                  ...
                  name={"message"}
                  placeholder={
                    "Enter your questions, comments, concerns, or feedback..."
                  }
                />
              </Form.Group>
              ...
              >
                ...
                <Form.Control
                  type={"email"}
                  name={"emailAddress"}
                  placeholder={`Only include if you'd like to hear back from me`}
                />
              </Form.Group>
            ...
            <InfoList>
              {sortByJoinIndex(players).map((player) => {
                return (
                  <li key={player.name}>
                    <InfoText>
                      <HypeText text={player.name.toUpperCase()} />
                    </InfoText>
                  </li>
                );
              })}
              ...
              <LeaderboardPlayerNames lg={"6"}>
                <InfoList>
                  {leaderboard.map((leader) => {
                    return (
                      <li key={leader.name}>
                        <InfoText>{leader.name.toUpperCase()}</InfoText>
                      </li>
                    );
                  })}
                </InfoList>
              ...
              <LeaderboardScores lg={"6"}>
                <InfoList>
                  {leaderboard.map((leader) => {
                    return (
                      <li key={leader.name}>
                        <InfoText>${leader.score}</InfoText>
                      </li>
                    );
                  })}
                </InfoList>
        ...
        <StartGameInputGroup className={"mb-3 justify-content-center"}>
          {!mute && (
            <Row style={{ gap: "8px" }}>
              ...
              <StartGameButton
                onClick={handleStartSelfHostedGame}
                variant={"outline-light"}
              >
                START SELF HOSTED GAME
              </StartGameButton>
            </Row>
          )}
        </StartGameInputGroup>
