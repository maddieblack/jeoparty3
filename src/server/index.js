...
const updatePlayerScore = (sessionName, socketId, value, isCorrect) => {
  ...
  if (_.get(gameSession.players, `[${socketId}]`)) {
    const newScore =
      _.get(gameSession.players, `[${socketId}].score`, 0) +
...
const updateCategories = (sessionName, categoryIndex, clueIndex) => {
  ...
  categories[categoryIndex].numCluesUsed =
    categories[categoryIndex].numCluesUsed + 1;
...
const handlePlayerDisconnection = (sessionName, socket) => {
  ...
  if (player && !_.isEmpty(player.name)) {
    ...
    disconnectionCache.put(
      playerId,
      sessionCache.get(sessionName).players[socket.id],
      RECONNECT_WINDOW
    );
    ...
    if (gameSession.currentGameState === GameState.LOBBY) {
      ...
      gameSession.currentGameState === GameState.BOARD &&
      socket.id === gameSession.boardController.id
    ) {
...
const handlePlayerReconnection = (socket) => {
  ...
  if (player && sessionCache.get(player.sessionName)) {
    ...
    socket.emit(
      "set_game_state",
      sessionCache.get(socket.sessionName).currentGameState,
      () => {
...
const checkBoardCompletion = (sessionName) => {
  ...
  if (gameSession.doubleJeoparty) {
    ...
    const sortedPlayers = _.cloneDeep(
      _.values(gameSession.players).sort((a, b) => b.score - a.score)
    );
    updateGameSession(
      sessionName,
      "boardController",
      getClient(sessionName, sortedPlayers[sortedPlayers.length - 1].socketId)
    );
  }
};
...
const checkBoardController = (sessionName) => {
  ...
  if (!_.get(gameSession.players, `[${gameSession.boardController.id}]`)) {
    const newBoardController = getClient(
      sessionName,
      _.keys(gameSession.players)[
        Math.floor(Math.random() * _.size(gameSession.players))
      ]
    );
  ...
  gameSession.clients.map((client) => {
    client.emit("set_game_state", GameState.BOARD, () => {
      ...
      client.emit(
        ...
        _.get(
          gameSession,
          `players[${gameSession.boardController.id}].name`,
          ""
        ),
      ...
      client.emit(
        "is_board_controller",
        client.id === gameSession.boardController.id,
        gameSession.boardRevealed
      );
  ...
  gameSession.clients.map((client) => {
    client.emit("set_game_state", GameState.WAGER, () => {
      if (gameSession.finalJeoparty) {
        ...
        const boardController = _.get(
          gameSession,
          `players[${gameSession.boardController.id}]`
        );
        ...
        client.emit(
          ...
          boardController,
          Math.max(
            _.get(boardController, "score", 0),
            gameSession.doubleJeoparty ? 2000 : 1000
          )
        );
        client.emit("player", _.get(gameSession, `players[${client.id}]`));
      }
...
const submitWager = (socket, wager) => {
  if (
    !sessionCache.get(socket.sessionName) ||
    sessionCache.get(socket.sessionName).currentGameState !== GameState.WAGER
  ) {
    return;
  }
  ...
  if (gameSession.finalJeoparty) {
    ...
    if (currentWagersSubmitted === totalWagers) {
      ...
      gameSession.browserClient.emit(
        "wagers_submitted",
        currentWagersSubmitted,
        totalWagers
      );
    }
  } else {
...
const updatePlayerWager = (socket, wager) => {
  ...
  const max = gameSession.finalJeoparty
    ? score
    : Math.max(score, gameSession.doubleJeoparty ? 2000 : 1000);
  updatePlayer(
    socket.sessionName,
    socket.id,
    "wager",
    formatWager(wager, min, max)
  );
};
...
const showClue = (sessionName, sayClueText) => {
  ...
  const clue = gameSession.finalJeoparty
    ...
    : gameSession.categories[gameSession.categoryIndex].clues[
        gameSession.clueIndex
      ];
  ...
  gameSession.clients.map((client) => {
    client.emit("set_game_state", GameState.CLUE, () => {
      ...
      client.emit(
        ...
        clue.dailyDouble ||
          gameSession.finalJeoparty ||
          gameSession.playersAnswered.includes(client.id)
      );
      client.emit("player", _.get(gameSession, `players[${client.id}]`));
    });
  });
...
const buzzIn = (socket) => {
  ...
  if (gameSession.buzzInTimeout) {
    ...
    const categoryYear = gameSession.finalJeoparty
      ...
      : gameSession.categories[
          gameSession.categoryIndex
        ].clues[0].airdate.slice(0, 4);
    ...
    gameSession.clients.map((client) => {
      client.emit("set_game_state", GameState.ANSWER, () => {
        const oldScore = _.get(
          gameSession,
          `players[${client.id}].oldScore`,
          0
        );
        ...
        client.emit(
          "player_name",
          _.get(gameSession, `players[${socket.id}].name`, "")
        );
        client.emit("answers_submitted", currentAnswersSubmitted, totalAnswers);
        client.emit(
          "is_answering",
          gameSession.finalJeoparty ? oldScore > 0 : client.id === socket.id,
          gameSession.finalJeoparty
        );
        client.emit("player", _.get(gameSession, `players[${client.id}]`));
      });
      ...
      if (gameSession.finalJeoparty) {
        gameSession.clients.map((client) => {
          ...
          if (oldScore > 0) {
            submitAnswer(
              client,
              _.get(gameSession, `players[${client.id}].answer`, ""),
              true
            );
          }
        });
        ...
        submitAnswer(
          socket,
          _.get(gameSession, `players[${socket.id}].answer`, ""),
          true
        );
      }
...
const startTimer = (sessionName) => {
  ...
  setTimeout(() => {
    ...
    if (gameSession.buzzInTimeout) {
      const correctAnswer =
        sessionCache.get(sessionName).categories[gameSession.categoryIndex]
          .clues[gameSession.clueIndex].answer;
...
const submitAnswer = (socket, answer, timeout) => {
  ...
  if (
    !gameSession.finalJeoparty &&
    gameSession.playersAnswered.includes(socket.id)
  ) {
    return;
  }
  ...
  if (
    ...
    (gameSession.finalJeoparty &&
      !_.get(
        gameSession,
        `players[${socket.id}].finalJeopartyAnswerSubmitted`,
        false
      ))
  ) {
  ...
  if (gameSession.finalJeoparty) {
    ...
    if (
      currentAnswersSubmitted === totalAnswers &&
      timeout &&
      gameSession.currentGameState !== GameState.DECISION
    ) {
      showFinalJeopartyDecision(socket.sessionName);
    } else {
      gameSession.browserClient.emit(
        "answers_submitted",
        currentAnswersSubmitted,
        totalAnswers
      );
    }
...
const showFinalJeopartyDecision = (sessionName) => {
  ...
  if (gameSession.finalJeopartyDecisionIndex < totalAnswers) {
    ...
    updateGameSession(
      sessionName,
      "finalJeopartyDecisionIndex",
      gameSession.finalJeopartyDecisionIndex + 1
    );
...
const showPodium = (sessionName, championOverride) => {
  ...
  const gameSession = sessionCache.get(sessionName);
  const champion = _.values(gameSession.players).sort(
    (a, b) => b.score - a.score
  )[0];
...
io.on("connection", (socket) => {
  ...
  socket.on("connect_device", async (isMobile) => {
    ...
    if (isMobile) {
      ...
      getRandomCategories(
        (categories, doubleJeopartyCategories, finalJeopartyClue) => {
          updateGameSession(socket.sessionName, "categories", categories);
          updateGameSession(
            socket.sessionName,
            "doubleJeopartyCategories",
            doubleJeopartyCategories
          );
          updateGameSession(
            socket.sessionName,
  ...
  socket.on("submit_signature", (playerName, signature) => {
    ...
    if (_.isEmpty(checkPlayerMessage)) {
      ...
      socket.emit(
        ...
        _.get(gameSession, `players[${socket.id}]`)
      );
      gameSession.browserClient.emit("players", gameSession.players);
  ...
  socket.on("start_game_self_hosted", () => {
    if (!sessionCache.get(socket.sessionName)) {
      return;
    }

    updateGameSession(socket.sessionName, "isSelfHosted", true);
    console.log("SOCKET SELF HOST", {
      session: sessionCache.get(socket.sessionName),
    });
  });
      ...
      if (gameSession.finalJeoparty) {
        gameSession.clients.map((client) => {
          ...
          if (
            ...
            !_.get(
              gameSession,
              `players[${client.id}].finalJeopartyWagerSubmitted`,
              false
            )
          ) {
            submitWager(
              client,
              _.get(gameSession, `players[${client.id}].wager`, 0)
            );
          }
        });
      } else {
        // Daily double
        submitWager(
          gameSession.boardController,
          _.get(
            gameSession,
            `players[${gameSession.boardController.id}].wager`,
            0
          )
        );
      }
  ...
  socket.on("wager_livefeed", (wagerLivefeed) => {
    ...
    sessionCache
      .get(socket.sessionName)
      .browserClient.emit("wager_livefeed", wagerLivefeed);
  });
  ...
  socket.on("answer_livefeed", (answerLivefeed) => {
    ...
    sessionCache
      .get(socket.sessionName)
      .browserClient.emit("answer_livefeed", answerLivefeed);
  });
  ...
  socket.on("show_decision", (isCorrect) => {
    ...
    if (gameSession.finalJeoparty) {
      ...
      if (isCorrect) {
        const newBoardController = getClient(
          socket.sessionName,
          gameSession.playersAnswered[gameSession.playersAnswered.length - 1]
        );
        ...
        _.size(gameSession.playersAnswered) === _.size(gameSession.players) ||
        clue.dailyDouble
      ) {
